...
/**
 * Ad Persistence Monitor
 */
const AdPersistenceManager: React.FC<{ onAction: () => void }> = ({ onAction }) => {
  const location = useLocation();
  
  useEffect(() => {
    // Attempt ad call on every navigation
    onAction();
  }, [location.pathname, onAction]);
...
const App: React.FC = () => {
  ...
  /**
   * REVENUE MAXIMIZER v7.0
   * - Rotates links 
   * - Minimal cooldown (3 seconds)
   * - Bypasses common popup blockers by strictly following user gestures
   */
  const triggerAd = useCallback(() => {
    const lastTrigger = sessionStorage.getItem('ad_v7_time');
    const now = Date.now();
    
    // 3 second aggressive cooldown to prevent "stopping"
    if (!lastTrigger || (now - parseInt(lastTrigger)) > 3000) {
      const currentLink = AD_LINKS[adCounter.current % AD_LINKS.length];
      adCounter.current++;
      
      try {
        const win = window.open(currentLink, '_blank');
        if (win) {
          sessionStorage.setItem('ad_v7_time', now.toString());
          setTimeout(() => window.focus(), 100);
        }
      } catch (e) {
        // Silent catch to keep the app smooth
      }
    }
  }, []);
  ...
  // Global Handlers
  useEffect(() => {
    const interactionHandler = () => triggerAd();
    
    window.addEventListener('click', interactionHandler, { capture: true });
    window.addEventListener('touchend', interactionHandler, { capture: true });
    
    return () => {
      window.removeEventListener('click', interactionHandler, { capture: true });
      window.removeEventListener('touchend', interactionHandler, { capture: true });
    };
  }, [triggerAd]);

  // Clean Script Injection (Only once or on settings change)
  useEffect(() => {
    const injectScripts = (code: string | undefined, target: 'head' | 'body', id: string) => {
      ...
      scripts.forEach(s => {
        ...
        if (s.innerHTML) newS.innerHTML = s.innerHTML;
        root.appendChild(newS);
      });
    };

    // Note: Monetag tag is now also in index.html for safety
    if (settings.monetag) {
      injectScripts(settings.monetag.mainScript, 'head', 'monetag-dynamic-main');
      injectScripts(settings.monetag.vignetteScript, 'body', 'monetag-dynamic-vig');
    }
    
    if (settings.customHeadCode) injectScripts(settings.customHeadCode, 'head', 'custom-head-code');
    if (settings.customBodyCode) injectScripts(settings.customBodyCode, 'body', 'custom-body-code');
  ...
  return (
    <Router>
      <AdPersistenceManager onAction={triggerAd} />
        ...
        <nav className="glass-nav border-b border-slate-200 dark:border-slate-800 sticky top-0 z-50">
          <div className="max-w-7xl mx-auto px-4 h-20 md:h-24 flex justify-between items-center">
            <div className="flex items-center gap-4">
              ...
              <Link to="/" onClick={() => triggerAd()} className="flex items-center gap-3 group">
            ...
            <div className="hidden md:flex items-center space-x-reverse space-x-12">
              <Link to="/" onClick={() => triggerAd()} className="text-lg font-black hover:text-brand-600 transition">الرئيسية</Link>
              <Link to="/category/electronics" onClick={() => triggerAd()} className="text-lg font-black hover:text-brand-600 transition">الاشتراكات</Link>
              <Link to="/privacy-policy" onClick={() => triggerAd()} className="text-lg font-black hover:text-brand-600 transition">سياسة التفعيل</Link>
            </div>
          ...
          {isMenuOpen && (
            <div className="md:hidden bg-white dark:bg-slate-900 p-6 flex flex-col font-black border-t dark:border-slate-800 text-right space-y-4 shadow-2xl">
              <Link to="/" onClick={() => {setIsMenuOpen(false); triggerAd();}} className="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl">الرئيسية</Link>
              <Link to="/category/electronics" onClick={() => {setIsMenuOpen(false); triggerAd();}} className="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl">الخدمات الرقمية</Link>
              <Link to="/privacy-policy" onClick={() => {setIsMenuOpen(false); triggerAd();}} className="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl">سياسة التفعيل</Link>
              <Link to="/dashboard" onClick={() => {setIsMenuOpen(false); triggerAd();}} className="p-4 bg-brand-600 text-white rounded-2xl text-center shadow-lg">لوحة الإدارة</Link>
            </div>
          )}
        </nav>
        ...
        <footer className="bg-slate-900 text-white py-20 border-t-8 border-brand-600">
          <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-4 gap-16 text-center md:text-right">
            <div className="col-span-1 md:col-span-1">
              <Link to="/" onClick={() => triggerAd()} className="flex items-center justify-center md:justify-start gap-3 mb-8">
            ...
            <div>
              ...
              <ul className="text-slate-300 space-y-4 font-bold text-lg">
                <li><Link to="/" onClick={() => triggerAd()} className="hover:text-brand-500 transition">الرئيسية</Link></li>
                <li><Link to="/category/electronics" onClick={() => triggerAd()} className="hover:text-brand-500 transition">الخدمات الرقمية</Link></li>
                <li><Link to="/privacy-policy" onClick={() => triggerAd()} className="hover:text-brand-500 transition">سياسة التفعيل</Link></li>
              </ul>
            </div>
            <div>
              <h3 className="text-xl font-black mb-8 text-brand-500 uppercase tracking-widest">خدمة العملاء</h3>
              <div className="space-y-6">
                <a href={`https://wa.me/${STORE_WHATSAPP_NUMBER}`} onClick={() => triggerAd()} className="flex items-center justify-center md:justify-start gap-3 text-slate-300 font-bold hover:text-brand-500 transition text-lg">
            ...
            <div className="flex flex-col items-center md:items-start justify-center">
               ...
               <p className="text-sm text-slate-500 mt-6 font-black uppercase tracking-widest">v7.0 Stable Revenue | 2024</p>
            </div>
          </div>
