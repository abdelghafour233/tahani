        ...
        <nav className="glass-nav border-b border-slate-200 dark:border-slate-800 sticky top-0 z-50">
          <div className="max-w-7xl mx-auto px-4 h-20 md:h-24 flex justify-between items-center">
            ...
            <div className="hidden md:flex items-center space-x-reverse space-x-12 font-black">
              ...
              <Link to="/category/electronics" className="hover:text-brand-600 transition">الاشتراكات</Link>
              <Link to="/privacy-policy" className="hover:text-brand-600 transition">سياسة التفعيل</Link>
            </div>
            ...
            <Route path="/dashboard/*" element={
              isAuthenticated ? (
                ...
                <div className="min-h-[70vh] flex items-center justify-center p-6 text-center">
                  <div className="bg-white dark:bg-slate-900 p-12 rounded-[45px] shadow-2xl w-full max-w-md">
                    <div className="bg-brand-600 w-24 h-24 rounded-3xl flex items-center justify-center mx-auto mb-8 shadow-2xl">
                      <Lock className="text-white" size={48} />
                    </div>
                    <h2 className="text-3xl font-black mb-8">دخول المسؤولين</h2>
                    <form onSubmit={handleLogin} className="space-y-6">
                      <input 
                        type={showPassword ? "text" : "password"}
                        className="w-full bg-slate-50 dark:bg-slate-800 border-2 border-transparent rounded-2xl px-6 py-5 focus:border-brand-500 outline-none transition text-center font-bold text-lg"
